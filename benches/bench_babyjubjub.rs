...
use bloock_babyjubjub_rs::Point;

fn criterion_benchmark(c: &mut Criterion) {
    let p: Point = Point {
        x: bloock_babyjubjub_rs::Fr::from_str(
            "17777552123799933955779906779655732241715742912184938656739573121738514868268",
        )
        .unwrap(),
        y: bloock_babyjubjub_rs::Fr::from_str(
    ...
    c.bench_function("point decompress", |b| {
        b.iter(|| bloock_babyjubjub_rs::decompress_point(p_comp))
    });

    let sk = bloock_babyjubjub_rs::new_key();
    let pk = sk.public();
    let msg = 5.to_bigint().unwrap();
    c.bench_function("sign", |b| b.iter(|| sk.sign(msg.clone())));
    let sig = sk.sign(msg.clone()).unwrap();
    c.bench_function("verify", |b| {
        b.iter(|| bloock_babyjubjub_rs::verify(pk.clone(), sig.clone(), msg.clone()))
    });
}
